...
func main() {
	...
	// 为何这里还需要for循环？
	// 若是 exit(0) 后，进程没退出，由接下来的代码确保进程一会退出
	// 就是for循环一直访问非法地址，正常情况下，一但出现非法地址访问，系统就会把该进程杀死，用这样的方法来确保进程退出。
	for {
		var x *int32
		*x = 0
	}
...
func schedinit() {
	...
	// 最大系统线程数量限制 10000，也就是限制 M 的数量
	// 可以通过 SetMaxThreads 调整
...
// 将 gp 的状态更改为 _Grunnable，以便调度器调度执行
// 并且如果 next==true，那么设置为优先级最高，并尝试 wakep
...
// 找不到工作的 m 就休眠了。
// 当其他线程发现有工作要做时，就会先找到空闲的 m，再通过 m.park 字段来唤醒本线程。
// 唤醒之后，回到 findrunnable 函数，继续寻找 goroutine，
// 找到后返回 schedule 函数，然后就会去运行找到的 goroutine。
//go:nosplit
func mPark() {
	g := getg()
	for {
...
func stopm() {
	...
	// 此时已经被唤醒，说明有任务要执行
	// 立即 acquire P
	// 因为当前唤醒的 M 主动找工作，所以 acquirep 并不需要创建 M，直接绑定 MP 即可
	acquirep(_g_.m.nextp.ptr())
	_g_.m.nextp = 0
}
...
func startm(_p_ *p, spinning bool) {
	...
	unlock(&sched.lock)
	// 到这里表示获取到了一个空闲 M
	if nmp.spinning { // 从 midle 中获取的 mp，不应该是 spinning 状态，获取的都是经过 stopm 的，stopm 之前都会退出 spinning
		throw("startm: m is spinning")
	}
	if nmp.nextp != 0 { // 这个位置是要留给参数_p_的，stopm 中如果被唤醒，则关联 nextp 和 m
		throw("startm: m has p")
	}
	if spinning && !runqempty(_p_) { // spinning 状态的 M 是在本地和全局都获取不到工作的情况，不能与spinning语义矛盾
...
func wakep() {
	...
	// 如果有其他的 M 处于自旋状态，那么就不管了，直接返回
	// 因为自旋的 M 会拼命找 G 来运行的，就不新找一个 M（劳动者）来运行了。
...
func execute(gp *g, inheritTime bool) {
	...
	_g_.m.curg = gp
	gp.m = _g_.m
	// goroutine 进入执行状态，将 gp 的状态改为 running
	...
	// 汇编实现的，从 g0 栈切换到 G 栈，然后 JMP G 任务函数代码
...
top:
	...
	// anyway.
	// 从 netpoll 任务获取
	// 从网络 IO 轮询器中找到就绪的 G，把这个 G 变为可运行的G
	if netpollinited() && atomic.Load(&netpollWaiters) > 0 && atomic.Load64(&sched.lastpoll) != 0 {
		if list := netpoll(0); !list.empty() { // non-blocking
			gp := list.pop()
			// 如果找到的可运行的网络 IO 的 G 列表，则把相关的 G 插入 `全局队列`
			injectglist(&list)
			// 更改 G 的状态为 _Grunnable，以便下次 M 能找到这些 G 来执行
			casgstatus(gp, _Gwaiting, _Grunnable)
			if trace.enabled {
				traceGoUnpark(gp, 0)
			}
	...
	// P.
	// 当前的 M 找不到 G 来运行。如果此时 P 处于 GC mark 阶段
...
top:
	...
	// findrunnable 如果获取不到可以执行的 G，会让 M 进入休眠，被唤醒后再重试
	if gp == nil {
...
func goexit0(gp *g) {
	...
	// goroutine 从执行状态 _Grunning，变成 _Gdead 状态
...
// 专门用于创建 G 的方法
// 新分配一个 g 对象，并申请好 stacksize 大小的栈，此时 g 的状态为 _Gidle
...
func newproc(fn *funcval) {
	...
	systemstack(func() {
		...
		// 将新的 goroutine 放入 P 的待运行本地队列
		_p_ := getg().m.p.ptr()
		runqput(_p_, newg, true)

		if mainStarted {
			// 尝试唤醒 M 来执行任务 (只是 M 需要 P 才可以运行)
			wakep()
		}
	})
}
...
func newproc1(fn *funcval, callergp *g, callerpc uintptr) *g {
	...
	// 初始化 G 的 sched，保存 sp，pc，任务函数等
	// 调度器需要依靠这些字段才能把 goroutine 调度到 CPU 上运行
	newg.sched.sp = sp
	newg.stktopsp = sp

	// 重点注意：
	// newg.sched.pc 表示当 newg 被调度起来运行时从这个地址开始执行指令
	// 注册 goroutine 执行完任务函数后，执行的 goexit 函数
	newg.sched.pc = abi.FuncPCABI0(goexit) + sys.PCQuantum // +PCQuantum so that previous instruction is in same function
	// sched.g 保存当前新的 G
	newg.sched.g = guintptr(unsafe.Pointer(newg))
	// 将当前的 pc 压入栈，保存 g 的任务函数为 pc
	gostartcallfn(&newg.sched, fn)

	// gopc 保存 newproc 的 pc
	newg.gopc = callerpc
	...
	// 判断 g 的任务函数是不是 runtime 系统的任务函数，是则 sched.ngsys 加1
...
// sched.lock must be held.
// 从全局队列获取可以运行的 G，并把一批 G 从全局队列转移到 P 的本地队列，避免锁开销
